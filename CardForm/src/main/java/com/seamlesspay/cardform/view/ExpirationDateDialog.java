...
import com.seamlesspay.cardform.utils.ExpirationDateDialogTheme;
import com.seamlesspay.cardform.utils.ExpirationDateItemAdapter;
...
public class ExpirationDateDialog
  ...
  private final int CURRENT_MONTH =
    Calendar.getInstance().get(Calendar.MONTH) + 1; // months are 0 indexed
  ...
  protected ExpirationDateDialog(
    Context context,
    boolean cancelable,
  ...
  public static ExpirationDateDialog create(
    Activity activity,
    ExpirationDateEditText editText
  ) {
    ExpirationDateDialogTheme theme = ExpirationDateDialogTheme.detectTheme(
      activity
    );
    ...
    if (theme == ExpirationDateDialogTheme.LIGHT) {
      dialog =
        new ExpirationDateDialog(
          activity,
          R.style.bt_expiration_date_dialog_light
        );
    } else {
      dialog =
        new ExpirationDateDialog(
          activity,
          R.style.bt_expiration_date_dialog_dark
        );
    }
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    final ExpirationDateItemAdapter monthAdapter = new ExpirationDateItemAdapter(
      getContext(),
      mTheme,
      MONTHS
    );

    final ExpirationDateItemAdapter yearAdapter = new ExpirationDateItemAdapter(
      getContext(),
      mTheme,
      mYears
    );
    ...
    monthAdapter.setOnItemClickListener(
    ...
    yearAdapter.setOnItemClickListener(
      new AdapterView.OnItemClickListener() {
        ...
        new Runnable() {
          ...
          public void run() {
            Activity ownerActivity =
              ExpirationDateDialog.this.getOwnerActivity();
            if (
              mEditText.isFocused() &&
              ownerActivity != null &&
  ...
  public boolean onTouchEvent(final MotionEvent event) {
    ...
    if (isShowing() && closeOnTouch) {
      View rootView = getOwnerActivity()
        .getWindow()
        .getDecorView()
        .getRootView();
  ...
  private boolean isOutOfBounds(MotionEvent event) {
    ...
    final int slope = ViewConfiguration
      .get(getContext())
    ...
    return (
      (x < -slope) ||
      (y < -slope) ||
      (x > (decorView.getWidth() + slope)) ||
      (y > (decorView.getHeight() + slope))
    );
  }
}

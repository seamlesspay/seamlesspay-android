...
public class SupportedCardTypesView extends TextView {
  ...
  public void setSelected(CardType... cardTypes) {
    ...
    SpannableString spannableString = new SpannableString(
      new String(new char[mSupportedCardTypes.size()])
    );
    ...
    for (int i = 0; i < mSupportedCardTypes.size(); i++) {
      span =
        new PaddedImageSpan(
          getContext(),
          mSupportedCardTypes.get(i).getFrontResource()
        );

      span.setDisabled(
        !Arrays.asList(cardTypes).contains(mSupportedCardTypes.get(i))
      );

...
public enum CardType {
  VISA("^4\\d*", R.drawable.bt_ic_visa, 16, 16, 3, "CVV", null),
  MASTERCARD(
  ...
  AMEX("^3[47]\\d*", R.drawable.bt_ic_amex, 15, 15, 4, "CID", null),
  DINERS_CLUB(
    "^(36|38|30[0-5])\\d*",
  ...
  JCB("^35\\d*", R.drawable.bt_ic_jcb, 16, 16, 3, "CVV", null),
  MAESTRO(
  ...
  CardType(
    ...
    mRelaxedPrefixPattern =
      relaxedPrefixPattern == null
        ? null
        : Pattern.compile(relaxedPrefixPattern);
   ...
   * for no match. A partial account type may be given, with the caveat that
   * it may not have enough digits to match.
   */
  public static CardType forCardNumber(String cardNumber) {
    ...
    CardType relaxedPrefixPatternMatch = forCardNumberRelaxedPrefixPattern(
      cardNumber
    );

    if (
      relaxedPrefixPatternMatch != EMPTY && relaxedPrefixPatternMatch != UNKNOWN
    ) {
  ...
  /**
   * @return The relaxed prefix regex matching this card type.
   * To be used in determining card type if no pattern matches.
   */
  ...
  /**
   * @return The android resource id for the front card image,
   * highlighting card number format.
   */
  ...
  /**
   * @return The android resource id for the security code
  ...
  public boolean validate(String cardNumber) {
    ...
    if (numberLength < mMinCardLength || numberLength > mMaxCardLength) {
      ...
      !mPattern.matcher(cardNumber).matches() &&
      mRelaxedPrefixPattern != null &&
      !mRelaxedPrefixPattern.matcher(cardNumber).matches()
    ) {
      return false;
    }

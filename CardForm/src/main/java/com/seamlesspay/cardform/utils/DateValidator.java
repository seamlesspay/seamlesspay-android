...
public class DateValidator {
  /**
   * Maximum amount of years in advance that a credit card expiration date
   * should be trusted to be valid. This is mostly used if the current date
   * is towards the end of the century and the expiration date is at the
   * start of the following one.
   *
   * Ex. Current year is 2099, Expiration date is "01/01". The YY is
   * "less than" the current year, but since the difference is less
   * than {@code MAXIMUM_VALID_YEAR_DIFFERENCE}, it should still
  ...
  /**
   * Used in tests to inject a custom {@link java.util.Calendar} to
   * stabilize dates. Normal usage will just delegate to the actual date.
   */
  ...
  protected boolean isValidHelper(String monthString, String yearString) {
    ...
    if (
      !TextUtils.isDigitsOnly(monthString) ||
      !TextUtils.isDigitsOnly(yearString)
    ) {
      return false;
    }
  ...
  /**
   * {@link java.util.Calendar#MONTH} is 0-prefixed. Add {@code 1} to align
  ...
  /**
   * {@link java.util.Calendar#YEAR} is the full, 4-digit year. Take the
   * trailing two digits to align it with visualized expiration dates.
   */

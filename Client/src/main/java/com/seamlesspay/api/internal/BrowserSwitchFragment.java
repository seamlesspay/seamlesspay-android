...
import com.seamlesspay.api.internal.BrowserSwitchActivity;
...
public abstract class BrowserSwitchFragment extends Fragment {
  ...
  private static final String EXTRA_REQUEST_CODE =
    "com.seamlesspay.browserswitch.EXTRA_REQUEST_CODE";
  ...
  public String getReturnUrlScheme() {
    return (
      mContext.getPackageName().toLowerCase().replace("_", "") +
      ".browserswitch"
    );
  }
  ...
  public void browserSwitch(int requestCode, Intent intent) {
    if (requestCode == Integer.MIN_VALUE) {
      BrowserSwitchResult result = BrowserSwitchResult.ERROR.setErrorMessage(
        "Request code cannot be Integer.MIN_VALUE"
      );
    ...
    if (!isReturnUrlSetup()) {
      BrowserSwitchResult result = BrowserSwitchResult.ERROR.setErrorMessage(
        ...
        "https://github.com/seamlesspay/seamlesspay-android for more " +
      ...
      BrowserSwitchResult result = BrowserSwitchResult.ERROR.setErrorMessage(
        String.format(
          "No installed activities can open this URL: %s",
   ...
   * @param returnUri The return uri. {@code null} unless the result is
   * {@link BrowserSwitchResult#OK}.
   */
  public abstract void onBrowserSwitchResult(
    int requestCode,

...
public abstract class BaseCardBuilder<T>
  ...
  /**
   * @param countryCode The ISO 3166-1 alpha-3 country
   * code specified in the card's billing address.
   * @return {@link BaseCardBuilder}
   */
  @SuppressWarnings("unchecked")
  ...
  protected void build(JSONObject json) throws JSONException {
    ...
    if (mExpirationMonth != null && mExpirationYear != null) {
      json.put(
        ...
        mExpirationMonth +
        "/" +
        mExpirationYear.substring(
          mExpirationYear.length() - 2,
          mExpirationYear.length()
        )
      );
    }
